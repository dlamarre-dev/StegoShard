...
- **The site can be found and described: sitemap, robots, `llms.txt`, and the metadata
  ...
  a canonical URL and Open Graph tags so a search result has a summary and a shared link
  has a preview. The preview image is generated by `npm run og` from the same brand
  primitives as the extension icons and the printed sheet, so there is one mark, not three.

  Two things were not what they looked like. **`robots.txt` at a project path does
  nothing**: crawlers read it only from the root of a host, which for a GitHub Pages
  project site is `dlamarre-dev.github.io/robots.txt`, and that was a 404. The file that
  is obeyed now lives in a separate user-site repository; the copy here is kept in step
  for the day a custom domain points at this project, and says so in its own text.

  And the home page **already had a description**, which the first version of this work
  did not notice, so it briefly shipped two different ones. The plugin no longer writes
  descriptions at all: it reads the title and description back out of each page and
  mirrors them into the Open Graph and Twitter tags, which is why they cannot drift, and
  fails the build for any page missing one. The legal pages gained the descriptions they
  never had, and the home page's kept wording, "robust", was corrected to "resilient" to
  match the rewording already made everywhere else.

  None of it reaches the offline bundle or the extension packages. The absolute URLs
  would be wrong in a `file://` copy, and `public/` is shared with the extension build,
  which is the trap the Search Console token avoided the same way.
