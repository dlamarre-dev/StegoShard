 ...
 *
 * Run with:
 *   npm run bench:stego -- --corpus <dir> [--pairs 3] [--label s0] [--seed <text>]
...
async function main(): Promise<void> {
  ...
  for (const file of files) {
    ...
    for (let p = 0; p < pairs; p++) {
      ...
      const stegoJpeg = await embedBytesStegoJpeg(coverJpeg, payload, posKey, GALLERY_EMBED_MARGIN);
      const embedMs = performance.now() - e0;
      sampleRss();
      writeFileSync(join(workDir, label, `${id}-stego-${p}.jpg`), stegoJpeg);

      const back = await extractBytesStegoJpeg(
        stegoJpeg,
        posKey,
        GALLERY_SLOT_BYTES,
        GALLERY_READ_MARGIN,
...
of ${r.slot.bytes} bytes (${r.slot.bits} bits) per pair, embed margin ${r.slot.embedMargin}.
