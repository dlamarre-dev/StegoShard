...
import { ACCENT_BOT, ACCENT_TOP, insideRoundedRect, insideStegosaurus } from '../src/core/brand';

const SIZES = [16, 32, 48, 128];
const SS = 4; // supersampling factor
const OUT = resolve(process.cwd(), 'public/icons');

// The mark itself (path, polygon, point tests) lives in src/core/brand.ts so the
// generated images and the PDF can stamp the same logo. Keep it in sync with the
// master public/icons/icon.svg.
const TOP = ACCENT_TOP;
const BOT = ACCENT_BOT;
