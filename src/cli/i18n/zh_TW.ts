...
export const zhTW: CliCatalog = {
  ...
  errExportNumberNotDeniable:
    '--export-number 不能與 {flags} 並用（可否認的保險庫不得攜帶任何將它與另一次匯出連結起來的東西）',
  errCoverReused:
    '這張照片在本次工作階段中已經以這個密碼藏過一個祕密。再次使用會同時削弱兩個藏匿處（SPEC §5.3）。請換一張照片或換一個密碼。若確實有意，可用 --allow-cover-reuse 覆寫。',
  ...
  helpExportNumber:
    '由你自己為這次匯出編號：還原時若顯示較小的編號，就表示那是較舊的副本（不可用於可否認的輸出）',
  helpForce: '覆寫既有的輸出檔（預設：拒絕）',
  helpQuiet: '不在 stderr 顯示進度',
  helpJson: '在 stdout 輸出一份 JSON 文件；進度以 NDJSON 輸出至 stderr',
  helpMcp: '透過 stdio 提供 Model Context Protocol，供 AI 代理使用（見 docs/API.md）',
