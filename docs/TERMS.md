# StegoShard terms of service

_Last updated: 2026-08-09_

StegoShard is free, open-source software provided under the [MIT License](../LICENSE).
By using the extension or the web app, you agree to these terms.

## The service

StegoShard encrypts a file on your device and encodes it into images or a binary
container you can store on disk or print. It runs entirely in your browser; there is
no application account or backend operated by us (see the
[Privacy Policy](PRIVACY.md)).

## No warranty

The software is provided "as is", without warranty of any kind, express or implied,
including but not limited to merchantability, fitness for a particular purpose, and
non-infringement. To the maximum extent permitted by law, the authors are not liable
for any claim, damages, or loss arising from the use of the software.

## Your responsibility

- You are responsible for your password and key. **If you lose your password or key,
  your data is permanently unrecoverable** — no one can restore it for you.
- Keep multiple copies of your recovery artifacts across independent locations. Do not
  rely on a single medium.
- Use StegoShard only for lawful purposes and only with data you are entitled to store.

## Changes

These terms may change; updates are published here. Continued use after a change
constitutes acceptance.

## Contact

<https://github.com/dlamarre-dev/StegoShard>
