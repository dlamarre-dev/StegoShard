...
- **DEK** (data encryption key): random 256-bit AES-GCM key, drawn as raw bytes
  from `randomBytes` and then `subtle.importKey`'d, **not** `subtle.generateKey`.
  That is deliberate: `generateKey` would bypass the single entropy tap described
  in §2, including the optional user-entropy layer. Encrypts the content
  envelope. Never stored in the clear; the keystore holds it wrapped.
...
(`crypto.getRandomValues`), through exactly one helper.**

- That helper is `randomBytes` (`src/core/crypto.ts`). It calls
  `crypto.getRandomValues`, in 64 KiB windows for requests above that cap, and
  then XORs in the optional user-entropy layer. It is **not** a bare wrapper, and
  that construction is the one hand-composed thing in the project: see §5.2, which
  gives the formula and the statistical testing behind it. The CSPRNG is consulted
  on every draw either way, so the layer can never subtract randomness.
- Consumers: KDF salt (16 B), wrap IV (12 B), content IV (12 B), set id (8 B,
  non-secret), **and the DEK**. `subtle.generateKey` is not used anywhere in the
  core, precisely so that nothing bypasses this tap.
